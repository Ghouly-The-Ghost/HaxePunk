...
class Mouse
{
	...
	public static inline function define(input:InputType, button:MouseButton)
	{
		...
		if (_buttonMap[button].indexOf(input) < 0) Map[button].push(input);
	}
	...
	static function onMouseDown(e:MouseEvent)
	{
		if (!mouseDown)
		{
			...
			mouseUp = false;
			mousePressed = true;
			if (_buttonMap.exists(MouseButton.LEFT_BUTTON)) for (input in _buttonMap[MouseButton.LEFT_BUTTON]) Input.triggerPress(input);
		}
	}
	...
	static function onMouseUp(e:MouseEvent)
	{
		...
		mouseUp = true;
		mouseReleased = true;
		if (_buttonMap.exists(MouseButton.LEFT_BUTTON)) for (input in _buttonMap[MouseButton.LEFT_BUTTON]) Input.triggerRelease(input);
	}
	...
	static function onMiddleMouseDown(e:MouseEvent)
	{
		if (!middleMouseDown)
		{
			...
			if (_buttonMap.exists(MouseButton.MIDDLE_BUTTON)) for (input in _buttonMap[MouseButton.MIDDLE_BUTTON]) Input.triggerPress(input);
		}
	}
	...
	static function onMiddleMouseUp(e:MouseEvent)
	{
		...
		if (_buttonMap.exists(MouseButton.MIDDLE_BUTTON)) for (input in _buttonMap[MouseButton.MIDDLE_BUTTON]) Input.triggerRelease(input);
	}
	...
	static function onRightMouseDown(e:MouseEvent)
	{
		if (!rightMouseDown)
		{
			...
			if (_buttonMap.exists(MouseButton.RIGHT_BUTTON)) for (input in _buttonMap[MouseButton.RIGHT_BUTTON]) Input.triggerPress(input);
		}
	}
	...
	static function onRightMouseUp(e:MouseEvent)
	{
		...
		if (_buttonMap.exists(MouseButton.RIGHT_BUTTON)) for (input in _buttonMap[MouseButton.RIGHT_BUTTON]) Input.triggerRelease(input);
	}
